...
def add_password(dash, refresh_callback):
    ...
    def save_entry():
        ...
        if not site or not user or not pwd:
            msg_label.configure(text="⚠️ Site, Username & Password required!", text_color="red")
            return
    ...
    ctk.CTkButton(
        btn_frame, text="💾 Save Entry", width=120,
...
def view_password(site, details, dash):
    ...
    def check_master(event=None):
        if entry_master.get() == MASTER_PASSWORD:
            ...
            ctk.CTkLabel(popup, text=f"🔑 {site} Password:", font=("Arial", 14, "bold")).pack(pady=10)
            ...
            btn_toggle = ctk.CTkButton(popup, text="👁 Show", width=80, command=toggle_show)
            btn_toggle.pack(pady=5)

            def copy_to_clipboard():
                pyperclip.copy(p)

            ctk.CTkButton(popup, text="📋 Copy to Clipboard", command=copy_to_clipboard).pack(pady=10)
        else:
            ctk.CTkLabel(auth_win, text="❌ Wrong Master Password!", text_color="red").pack(pady=5)
...
def open_dashboard(root):
    root.destroy()
    dash = ctk.CTk()
    dash.title("🔐 SecureDrive - Password Manager")
    dash.geometry("600x400")

    title = ctk.CTkLabel(dash, text="🔐 SecureDrive Dashboard", font=("Arial", 20, "bold"))
    title.pack(pady=15)
    ...
    ctk.CTkButton(dash, text="➕ Add Entry", command=lambda: add_password(dash, refresh)).pack(pady=10)
...
def login_screen():
    ...
    def check_login(event=None):
        if entry.get() == MASTER_PASSWORD:
            ...
            msg.configure(text="❌ Incorrect Password", text_color="red")
